...
function MockApp() {
  ...
  return (
    <div className="min-h-screen bg-bg-default">
      <header className="sticky top-0 z-50 bg-brand-primary text-ink-inverse px-4 py-3 flex flex-wrap items-center gap-3 shadow-md">
        <h1 className="type-h3 font-bold">{t('app.name')} — Souq v1</h1>
        <div className="flex flex-wrap gap-2 ms-auto items-center">
          <button
            type="button"
            onClick={() => setViewMode(viewMode === 'gallery' ? 'device' : 'gallery')}
            className="type-body-sm px-3 py-2 rounded-lg bg-bg-surface/20 text-ink-inverse min-h-[44px]"
          >
          ...
          <select
            ...
            className="type-body-sm px-2 py-2 rounded-lg"
          >
          ...
          <button
            ...
            className="type-body-sm px-3 py-2 rounded-lg bg-brand-accent text-ink-primary min-h-[44px] font-semibold"
          >
            {locale === 'ar' ? 'EN' : 'AR'}
          </button>
        </div>
